  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="border border-dotted border-white/20 text-white/60 text-xs px-1.5 py-0.5 font-mono">v{product.versions[0]?.version || 1}</span>
            <StatusBadge status={product.status} />
          </div>
          <div className="mt-2 flex items-center gap-4">
            <CopyableId label="ID" value={product.id} size="sm" />
            <span className="font-mono text-xs text-white/40">code: {product.code}</span>
          </div>
        </div>
        <button
          onClick={() => setShowAddPrice(true)}
          className="bg-white text-black px-4 py-2 font-mono text-xs uppercase tracking-wide hover:bg-white/90"
        >
          Configure Pricing →
        </button>
      </div>
      ...
      <div>
            ...
            {price.tiers && price.tiers.length > 0 && (
              <div className="mt-3 ml-6 space-y-1">
                {price.tiers.map((tier, i) => (
                  <div key={i} className="font-mono text-xs text-white/50">
        ...
        <button
          onClick={() => setShowAddPrice(true)}
          className="mt-8 border border-dotted border-white/30 bg-transparent px-4 py-2 font-mono text-xs uppercase tracking-wide text-white hover:bg-white/5"
        >
          + Add Price
        </button>
      </div>
              ...
              <tbody>
                {subscribers.map((c) => (
                  <tr key={c.id} className="border-b border-dotted border-white/15 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-4 font-mono text-sm font-light text-white/60">{c.email}</td>
                    <td className="px-4 py-4 text-right font-mono text-xs text-white/40">{c.external_id}</td>
                  </tr>
                ))}
              </tbody>
            </table>
